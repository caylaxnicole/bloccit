...
describe("routes : flairs", () => {
  ...
  beforeEach((done) => {
    ...
    sequelize.sync({force: true}).then((res) => {
      Topic.create({
        title: "Sailing",
        description: "Post your sailing stories here."
      })
      .then((topic) => {
        this.topic = topic;
        Flair.create({
          name: "Catalina Sail Boats",
          color: "Red",
  ...
  describe("POST /topics/:topicId/flairs/create", () => {
   it("should create a new flair and redirect", (done) => {
      const options = {
        ...
        form: {
          name: "Sailing in the ocean",
          color: "Blue"
        }
      };
      request.post(options,
        (err, res, body) => {
          Flair.findOne({where: {name: "Sailing in the ocean"}})
          .then((flair) => {
            expect(flair).not.toBeNull();
            expect(flair.name).toBe("Sailing in the ocean");
  ...
  describe("GET /topics/:topicId/flairs/:id", () => {
     it("should render a view with the selected flair", (done) => {
       request.get(`${base}/${this.topic.id}/flairs/${this.flair.id}`, (err, res, body) => {
         expect(err).toBeNull();
         expect(body).toContain("Catalina Sail Boats");
         done();
       });
     });
   });
